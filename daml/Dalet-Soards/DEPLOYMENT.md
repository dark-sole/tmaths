# Dalet Option Pricer â€” Canton Network Deployment Guide
...
### Validator Node (Production â€” DevNet/TestNet/MainNet)
...
| **OS** | macOS, Linux, or Windows (macOS current â€” fine) |
...
> **Prerequisite:** A running Canton Network validator node is required before proceeding. See `VALIDATOR_SETUP.md` for the complete node setup guide (Mac → DigitalOcean).

**Goal:** Connect to the real Global Synchronizer infrastructure. Start minting validator liveness rewards.

**Timeline:** 2â€“7 days (whitelisting), then 1â€“2 days setup

### Prerequisites from Sponsor

You need from your sponsoring Super Validator:

1. **IP whitelisting** â€” provide your fixed egress IP; sponsor submits to Super Validators
2. **Onboarding secret** â€” one-time use, expires 48 hours (on DevNet, can self-generate via API, valid 1 hour)
3. **Sponsor SV URL** â€” format: `https://sv.sv-1.unknown_cluster.global.canton.network.YOUR_SV_SPONSOR`
...
|------|---------|---------|
| **Client Credentials Grant** | Machine-to-machine auth between validator components | Validator app backend â†’ Canton participant |
...
# Vet the package (topology transaction â€” recorded on ledger)
...
```scala
// In Canton Console â€” create the service
...
**Timeline:** 1â€“2 weeks (Tokenomics Committee review)
...
3. **Deploy:** Production-hardened configuration:
   ...
   - Regular database backups (critical â€” Super Validators cannot recover your app data)
...
- [ ] Key backup stored securely â€” loss of keys = loss of CC
...
### Separate Participant (production â€” each client runs their own node)
...
4. Operator creates `ServiceProposal` â†’ Client exercises `AcceptService`
...
|-------|----------|---------|
| LocalNet (Quickstart) | 1â€“3 days | None â€” runs locally |
| DevNet whitelisting | 2â€“7 days | Sponsor submits IP |
| DevNet live | 1â€“2 days | Onboarding secret |
| TestNet approval | 1â€“2 weeks | GSF Committee review |
| MainNet approval | 1â€“2 weeks | GSF Committee review |
| Featured app status | 1â€“2 weeks | GSF Committee review |
| **Total** | **~4â€“8 weeks** | Sponsorship (secured) |

---

*Tokenisys (The Dark Sole Enterprise Ltd) â€” February 2026*
